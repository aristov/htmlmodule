...
describe('HTMLDOM library', () => {
    ...
    describe('Event handlers', () => {
        it('onblur', () => {
            ...
            const element = htmldom('button', { onblur });

            document.body.appendChild(element);
            element.focus();
            assert(onblur.notCalled, 'not called before a blur');
            element.blur();
            assert(onblur.calledOnce, 'called once after a single blur');
            element.blur();
            assert(onblur.calledOnce, 'called once after the second blur');
            document.body.removeChild(element);
        });
        it('change', () => {
            const onchange = sinon.spy();
            const element = htmldom('input', { type : 'checkbox', onchange });

            document.body.appendChild(element);
            assert(onchange.notCalled, 'not called before a click');
            assert(!element.checked, 'not checked before a click');
            element.click();
            assert(onchange.calledOnce, 'called once after a single click');
            assert(element.checked, 'checked after a single click');
            element.click();
            assert(onchange.calledTwice, 'called twice after the second click');
            assert(!element.checked, 'not checked after the click');
            document.body.removeChild(element);
        });
        it('onclick', () => {
            ...
            document.body.appendChild(element);
            assert(onclick.notCalled, 'not called before a click');
            element.click();
            assert(onclick.calledOnce, 'called once after a single click');
            element.click();
            assert(onclick.calledTwice, 'called twice after the second click');
            document.body.removeChild(element);
        });
        it('ondblclick', () => {
            const ondblclick = sinon.spy();
            const element = htmldom('button', { ondblclick });

            document.body.appendChild(element);
            assert(ondblclick.notCalled, 'not called before a double click');
            element.dispatchEvent(new CustomEvent('dblclick'));
            assert(ondblclick.calledOnce, 'called once after a double click');
            element.dispatchEvent(new CustomEvent('dblclick'));
            assert(ondblclick.calledTwice, 'called twice after the second double click');
            document.body.removeChild(element);
        });
        it('onfocus', () => {
            const onfocus = sinon.spy();
            const element = htmldom('button', { onfocus });

            document.body.appendChild(element);
            assert(onfocus.notCalled, 'not called before a focus');
            element.focus();
            assert(onfocus.calledOnce, 'called once after a single focus');
            element.focus();
            assert(onfocus.calledOnce, 'called once after the second focus');
            document.body.removeChild(element);
        });
        it('oninvalid', () => {
            const oninvalid = sinon.spy();
            const element = htmldom('form', {
                children : htmldom('input', {
                    required : true,
                    oninvalid
                })
            });
            assert(oninvalid.notCalled, 'not called before a validation');
            element.checkValidity();
            assert(oninvalid.calledOnce, 'called once after a validation');
        });
        it('onreset', () => {
            const onreset = sinon.spy();
            const checkbox = htmldom('input', {
                type : 'checkbox',
                checked : true
            });
            const element = htmldom('form', {
            ...
            assert(onreset.notCalled, 'not called before a form reset');
            assert(checkbox.checked, 'checked before a form reset');
            element.reset();
            assert(onreset.calledOnce, 'called once after a form reset');
            assert(!checkbox.checked, 'not checked before a form reset');
        });
        it('onsubmit', () => {
            const onsubmit = sinon.spy();
            const children = htmldom('button');
            htmldom('form', { onsubmit, children });
            assert(onsubmit.notCalled, 'not called before the submit click');
            children.click();
            assert(onsubmit.calledOnce, 'called once after the submit click');
        });
        it('ontoggle', () => {
            const ontoggle = sinon.spy();
            const children = htmldom('summary');
            const element = htmldom('details', { ontoggle, children });
            assert(ontoggle.notCalled, 'not called before the summary click');
            element.dispatchEvent(new CustomEvent('toggle'));
            assert(ontoggle.calledOnce, 'called once after a toggle');
        });
    });
