...
describe('HTMLDOM library', () => {
    ...
    describe('Other', () => {
        ...
        it('abbr', () => {
            ...
            assert(htmlEqualMarkup(node, '<abbr title="Hyper text markup language">HTML</abbr>'), 'properly build node');
        });
        it('address', () => {
            const node = address('vasily@pupkin.ru');
            assert(htmlEqualMarkup(node, '<address>vasily@pupkin.ru</address>'), 'properly build node');
        });
    });
});
