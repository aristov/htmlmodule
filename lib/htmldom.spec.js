...
const {
    ...
    HTMLFieldSetElement,
    HTMLFormElement,
    HTMLHeadingElement, HTMLHeadElement, HTMLHtmlElement,
    HTMLInputElement,
    HTMLModElement,
...
describe('HTMLDOM library', () => {
    ...
    describe('Single elements', () => {
        ...
        it('embed', () => {
            ...
            assert(node.hasAttributes(), 'has attributes');
            assert(node.src, '/data/app.swf');
            assert(node.type, 'application/x-shockwave-flash');
        ...
        it('fieldset', () => {
            ...
            assert(node.name, 'geolocation');

            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'Field set');
        });
        it('figcaption', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'Image 1.1');

            assert(node.outerHTML, '<figcation>Image 1.1</figcation>');
        });
        it('figure', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, '`Twas brillig, and the slithy toves');

            assert(node.outerHTML, '<figure>`Twas brillig, and the slithy toves</figure>');
        });
        it('footer', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'Navigation menu, small text, copyright and contact information');

            assert(node.outerHTML, '<footer>Navigation menu, small text, copyright and contact information</footer>');
        });
        it('form', () => {
            const node = form({
                acceptCharset : 'utf-8',
            ...
            assert(node.hasAttributes(), 'has attributes');
            assert(node.attributes.length, 8);
            assert(node.acceptCharset, 'utf-8');
            assert(node.action, '/app/save');
            assert(node.autocomplete, 'off');
            assert(node.enctype, 'multipart-formdata');
            assert(node.method, 'post');
            assert(node.name, 'saveform');
            assert(node.noValidate, true);
            assert(node.target, '_top');

            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'Form controls here');
        });
        it('h1', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'HTML Standard');

            assert(node.outerHTML, '<h1>HTML Standard</h1>');
        });
        it('h2', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'Full table of contents');

            assert(node.outerHTML, '<h2>Full table of contents</h2>');
        });
        it('h3', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'Introduction');

            assert(node.outerHTML, '<h3>Introduction</h3>');
        });
        it('h4', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, '2.4 Common microsyntaxes');

            assert(node.outerHTML, '<h4>2.4 Common microsyntaxes</h4>');
        });
        it('h5', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, '2.4.4 Numbers');

            assert(node.outerHTML, '<h5>2.4.4 Numbers</h5>');
        });
        it('h6', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, '2.4.4.4 Percentages and lengths');

            assert(node.outerHTML, '<h6>2.4.4.4 Percentages and lengths</h6>');
        });
        it('head', () => {
            const node = head();
            ...
            assert(node.outerHTML, '<head></head>');
        });
        it('header', () => {
            ...
            assert(node.hasChildNodes(), 'has child nodes');
            assert(node.childNodes.length, 1);
            assert(node.textContent, 'The main heading, site navigation and search');

            assert(node.outerHTML, '<header>The main heading, site navigation and search</header>');
        });
        it('hgroup', () => {
            const node = hgroup();
            ...
            assert(node.outerHTML, '<hgroup></hgroup>');
        });
    });
