...
class EventType
{
  ...
  static get(elem, name) {
    return elem.__handlers.get(name) || null
  }
  ...
  static set(elem, name, callback) {
    if(!callback) {
      const listener = elem.__handlers.get(name)
      if(listener) {
        listener.__on = false
      }
      return
    ...
    const listener = e => {
      return callback.call(elem, e, elem.__getElemByNode(e.target))
    }
    listener.__on = true
    elem.__handlers.set(name, listener)
  }
