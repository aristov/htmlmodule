...
class ElemType
{
  ...
  __resetProps() {
    ...
    let name, type
    for(name in this.__vnode) {
      type = map[name]
      type.reset(this, name)
    }
  }
  ...
  static __defineProps(props) {
    ...
    for(prop in props) {
      ...
      if(typeof value === 'string') {
        value = props[prop] = AttrType.define(value)
      }
  ...
  static __init() {
    ...
    this.__sampleNode = this.namespace === null ?
      document.createElement(this.tagName) :
      document.createElementNS(this.namespace, this.tagName)
  }

  static events = Object.keys(EventType.types)
}
