...
const childFilterSet = new Set([false, null, undefined])
...
class ElemType
{
   ...
   */
  _setChildren() {
    this.children = [this.render()].flat(Infinity).filter(child => !childFilterSet.has(child))
    const indexA = {}
    const indexB = {}
    ...
    for(childA of childrenA) {
      indexB[childA.key] || childA.destroy()
    }
