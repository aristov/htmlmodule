...
import { AriaType } from './AriaType.js'
...
const { Comment, CustomEvent, document } = window
...
export class ElemType
{
    ...
    for(child of this.children) {
      ...
      if(child instanceof type) {
        if(typeof filter !== 'function') {
          filter = ElemType.__getFilterFn(filter)
        }
   ...
   * @private
   */
  static __getFilterFn(filter) {
    if(typeof filter === 'string') {
      return elem => elem[filter]
    }
