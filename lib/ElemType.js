...
class ElemType
{
   ...
   */
  setAttr(attr, value) {
    const attributes = this.__props.attributes
    if(value === null) {
  ...
  __updateProps() {
    ...
    const node = this.node
    const props = this.__props
    const prevProps = this.__prevProps || { attributes : {} }
    const handlers = {
      classList() {
        void null
      },
      ...
      attributes() {
        for(attr in props.attributes) {
          ...
          else if(value !== prevProps.attributes[attr]) {
      ...
      dataset() {
        for(attr in props.dataset) {
          ...
          else if(value !== prevProps.dataset?.[attr]) {
            node.dataset[attr] = value
          }
        }
      },
      style() {
        if(typeof props.style !== 'string' && props.style !== null) {
          Object.assign(sampleNode.style, props.style)
          value = sampleNode.getAttribute('style')
        }
        else value = props.style ?? ''
        if(!value.length) {
          delete props.style
          node.removeAttribute('style')
        }
        else if(value !== prevProps.style) {
          props.style = value
          node.style = value
        }
        sampleNode.style = ''
   ...
   */
  get attributes() {
    return this.__props.attributes
  }
  ...
  set attributes(attributes) {
    const props = this.__props
    if(attributes) {
      attributes = Object.assign(props.attributes, attributes)
