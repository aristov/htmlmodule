...
class ElemType
{
  ...
  _setChildren() {
    ...
    let nextA, i, j
    for(childA of childrenA) {
      indexB[childA.props.key] || childA._destroy()
    }
    for(i = 0; i < length; i++) {
      childB = childrenB[i]
  ...
  _setChildNodes(childrenB) {
    ...
    for(i = 0; i < length; i++) {
      ...
      if(childA === undefined) {
        childB._init()
        this.node.append(childB._getAnchorNode(this) || childB.node)
        mount(childB)
        continue
      }
      if(childB === undefined) {
        childA._destroyParents?.()
        childA._destroy()
        continue
      }
      ...
      childB._init()
      nodeA.replaceWith(childB._getAnchorNode(this) || childB.node)
      mount(childB)
    }
    this.children = childrenB
  }
  ...
  _destroyParents() {
    if(this.parent?.props) {
      this.parent._destroyParents(true)
    }
    if(!arguments[0]) {
      return
    }
