...
class ElemType
{
  ...
  _init(node) {
    ...
    if(this.parent !== undefined) {
      this.__node = document.createComment(this.constructor.name)
      this.__node.__instance = this
      node?.parentNode && node.replaceWith(this.__node)
  ...
  _setChildren(children) {
    ...
    if(!childNodes.length) {
      for(child of childrenB) {
        if(child.props) {
          child._init()
          node.append(child.__node || child.node)
          mount(child)
        }
  ...
  _setChildNodes(childrenB) {
    ...
    const length = Math.max(childNodes.length, childrenB.length)
    ...
    for(i = 0; i < length; i++) {
      ...
      if(childA === undefined) {
        childB._init?.()
        this.node.append(childB.__node || childB.node || childB)
        mount(childB)
        continue
      }
      if(childB === undefined) {
      ...
      childB._init?.()
      nodeA.replaceWith(childB.__node || childB.node || childB)
      mount(childB)
    }
    this.children = childrenB
  }
  ...
  static render(props, parentNode) {
    ...
    elem._init(node)
    parentNode?.append(elem.__node || elem.node)
    parent(elem)
    node || elem.mount()
    return elem
  }
