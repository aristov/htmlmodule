...
class ElemType
{
  ...
  __destroyParent() {
    if(this.parent?.props) {
      this.__loop = true
      this.parent.__destroyParent(true)
      this.__loop = false
    }
    if(!arguments[0]) {
      return
  ...
  __destroyChildren() {
    ...
    this.children?.forEach(child => child.__destroyChildren?.(true))
    if(!arguments[0]) {
      return
    }
    this.destroy()
