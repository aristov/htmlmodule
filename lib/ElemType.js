...
class ElemType
{
  ...
  __resetProps() {
    ...
    for(name in vnodeA) {
      ...
      else attr && node.removeAttribute(attr)
    }
  }
  ...
  __updateProps() {
    ...
    const handlers = {
      ...
      attributes() {
        for(attr in vnodeB.attributes) {
          value = vnodeB.attributes[attr]
          if(value === null) {
            node.removeAttribute(attr)
          }
          else if(value !== vnodeA?.attributes[attr]) {
            node.setAttribute(attr, value)
          }
        }
      },
      dataset() {
        for(attr in vnodeB.dataset) {
          value = vnodeB.dataset[attr]
          if(value === null) {
            delete node.dataset[attr]
          }
          else if(value !== vnodeA?.dataset?.[attr]) {
            node.dataset[attr] = value
          }
        }
      },
      style() {
        if(typeof vnodeB.style !== 'string' && vnodeB.style !== null) {
          Object.assign(sampleNode.style, vnodeB.style)
          value = sampleNode.getAttribute('style')
        }
        else value = vnodeB.style ?? ''
        if(!value.length) {
          delete vnodeB.style
          node.removeAttribute('style')
        }
        else if(value !== vnodeA?.style) {
          vnodeB.style = value
          node.style = value
        }
        sampleNode.style = ''
   ...
   */
  __setChildren() {
    if(this.children === null || this.__children === undefined) {
      return
    }
    const childrenB = []
    const node = this.node
  ...
  set innerHTML(innerHTML) {
    this.children = null
    this.vnode.innerHTML = innerHTML
  }
