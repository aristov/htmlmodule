...
class ElemType
{
  ...
  constructor(props = {}) {
    ...
    if(!constructor.hasOwnProperty('__props')) {
      constructor.__init()
    }
  ...
  __setClassName() {
    ...
    this.__setClassList(className || this.constructor.__getClassName())
  }
  ...
  __setClassList(className) {
    ...
    else classList.push(...list.filter(Boolean))
    if(!classList.length) {
      this.node.removeAttribute('class')
      return
    }
    const value = classList.join(' ')
    if(value !== this.node.getAttribute('class')) {
   ...
   */
  static __defineMethods(methods) {
  ...
  static __init() {
    ...
    if(this.hasOwnProperty('methods')) {
      this.__defineMethods(this.methods)
    }
